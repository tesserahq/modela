...
|--------|------|-------------|
| `PUT` | `/v1/keys/{provider}` | Register or rotate a key for the caller's project + provider |
| `GET` | `/v1/keys` | List registered providers for the caller's project (key values never returned) |
| `DELETE` | `/v1/keys/{provider}` | Revoke the key for this provider |

**`PUT /v1/keys/{provider}` request body:**
```json
{
  "api_key": "sk-proj-..."
}
...
{
  "keys": [
    { "provider": "openai", "key_hint": "...a3Kx", "updated_at": "2026-01-15T10:00:00Z" },
    { "provider": "anthropic", "key_hint": "...9mZp", "updated_at": "2026-02-01T08:30:00Z" }
  ]
}
```
...
| Method | Path | Description |
|--------|------|-------------|
| `GET` | `/v1/admin/projects/{project_id}/keys` | List registered providers for a project |
| `DELETE` | `/v1/admin/projects/{project_id}/keys/{provider}` | Revoke a project's key |
...
- Encrypt on `PUT /v1/keys/{provider}` before writing to DB
...
- OpenAI: `GET /v1/models` (cheapest available check)
- Anthropic: minimal `POST /v1/messages` with `max_tokens: 1`
